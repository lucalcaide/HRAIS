...
const FilesContract = () => {
  ...
  const handleDelete = (id, emp_no) => {
    ...
    if (confirmDelete) {
      axios.delete(`https://mysql-deploy-heroku-a202c0289f39.herokuapp.com/auth/delete_contract/${id}`)
        .then(result => {
          if (result.data.Status) {
            // Update state first
  ...
  const handleFileChange = (id, emp_no, event) => {
    ...
    if (file) {
      ...
      axios.post(`https://mysql-deploy-heroku-a202c0289f39.herokuapp.com/auth/upload_contract/${id}`, formData, {
        headers: {
                    ...
                    {currentEmployees.map(e => (
                      <tr key={e.id}>
                        ...
                        <td style={{ fontSize: '18px' }}>
                          {e.contract ? (
                            <a href={`https://mysql-deploy-heroku-a202c0289f39.herokuapp.com/Contracts/` + e.contract} className='btn btn-file-color btn-file-text rounded-pill' target="_blank" rel="noopener noreferrer">View Contract</a>
                          ) : (
