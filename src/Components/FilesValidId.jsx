...
const FilesValidId = () => {
  ...
  const handleDelete = (id, emp_no) => {
    ...
    if (confirmDelete) {
      axios.delete(`https://mysql-deploy-heroku-a202c0289f39.herokuapp.com/auth/delete_valid_id/${id}`)
        .then(result => {
          if (result.data.Status) {
            // Update state first
  ...
  const handleFileChange = (id, emp_no, event) => {
    ...
    if (file) {
      ...
      axios.post(`https://mysql-deploy-heroku-a202c0289f39.herokuapp.com/auth/upload_valid_id/${id}`, formData, {
        headers: {
                    ...
                    {currentEmployees.map(e => (
                      <tr key={e.id}>
                        ...
                        <td style={{ fontSize: '18px' }}>
                          {e.valid_id ? (
                            <a href={`https://mysql-deploy-heroku-a202c0289f39.herokuapp.com/IDs/` + e.valid_id} className='btn btn-file-color btn-file-text rounded-pill' target="_blank" rel="noopener noreferrer">View IDs</a>
                          ) : (
